...
public class EnviromentSpawn : MonoBehaviour
{
    ...
    void SpawnPlants()
    {
        if (Mathf.Abs(player.transform.position.y - lastDecoration) < 12f)
        {
            ...
            GameObject decoration = Instantiate(forest[Random.Range(0, forest.Count)]);

            if (Random.Range(0, 2) == 0) decoration.transform.position = new Vector3(-4, lastDecoration + Random.Range(3, 6), 1.3f);
            else decoration.transform.position = new Vector3(5, lastDecoration + Random.Range(3, 6), 3.5f);

            decoration.transform.parent = this.transform;
            if (decoration.transform.position.x == 5) decoration.transform.rotation = Quaternion.Euler(0f, 180, 0f);
            lastDecoration = decoration.transform.position.y;
